...
const TreatmentPlanContainer = () => {
  ...
  const planDetails = useSelector(
    (state) => state.treatmentplanAndComments?.planDetails
  ); //source of truth for loaded plan
  ...
  const getPlanDetails = (activeKeyId) => {
    ...
    dispatch(
      getPlanDetailsAndCommentsAction(
        actionTypes.SET_PLAN_DETAILS,
        'GET_TREATMENT_PLAN_DETAILS',
        [patientID, activeKeyId, rebootID],
        queryParams
      )
    );
  };
    ...
    if (
      ...
      switch (true) {
        case searchParams.has('latest'): {
          ...
          const { id, treatmentPlanStatus, treatmentPlans } =
            treatmentPlanLatest;
          setTabs(treatmentPlans);
          if (
            JSON.stringify(redirectionInfo) !== '{}' &&
            redirectionInfo['latest'] !== undefined
          ) {
        ...
        case searchParams.has('history'): {
          ...
          const treatmentPlans = treatmentPlanHistory?.find(
            (plan) => plan.id === +paramId
          )?.treatmentPlans;
          setTabs(() => treatmentPlans);
          setActiveKey(treatmentPlans[0]?.id);
          break;
        ...
        case searchParams.has('draft'): {
          if (
            ...
            navigate(
              `/patientDetails/${patientID}/treatmentPlan/LatestPlan?latest=0`
            );
          }
          ...
          if (
            JSON.stringify(redirectionInfo) !== '{}' &&
            redirectionInfo['draft'] !== undefined
          ) {
            tabActiveKey =
              redirectionInfo['draft'] === 'latest'
                ? treatmentPlans[treatmentPlans?.length - 1]?.id
                : redirectionInfo['draft'];
            if (redirectionInfo['draft'] !== 'latest')
              getPlanDetails(tabActiveKey);
          }
          setRedirectionInfo({});
          setActiveKey(tabActiveKey);
        ...
        case pathname.includes('noPlan') &&
          JSON.stringify(redirectionInfo) !== '{}' &&
          redirectionInfo['draft'] !== undefined: {
          navigate(
            `/patientDetails/${patientID}/treatmentPlan/DraftPlans?draft=0`
          );
          break;
        }
        default:
  ...
  useEffect(() => {
    if (planDetails?.result === 'success' && planDetails.data !== undefined) {
  ...
  if (planType === 'noPlan' && !isEdit) {
    ...
    return (
      <FillerPage message={'No plan shared yet.'} btnDetails={btnDetails} />
    );
  }
