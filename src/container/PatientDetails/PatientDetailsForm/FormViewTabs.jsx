...
function FormViewTabs() {
  ...
  const fetchedPatientDetails = useSelector(
    (state) => state.getPatientDetails.patientDetails
  );
  ...
  const cancelHandler = useCallback(() => {
    setIsEdit(false);
    setDentalHistory(
      fetchedPatientDetails.data?.patientPreviousDentalHistoryDetails ?? {}
    ); //set to redux state value
    setTreatmentGoalForm(
      convertFormat(fetchedPatientDetails.data?.patientTreatmentGoal) ?? {}
    ); //set to redux state value
  ...
  const getPatientDetails = () => {
    dispatch(
      getPatientDetailsAction('GET_PATIENT_DETAILS', [patientID, rebootID])
    );
  };
  ...
  const addPatientDetails = () => {
    ...
    putCall(payload, 'UPDATE_PATIENT_DETAILS', [rebootID], params).then(
      (data) => {
  ...
  useEffect(() => {
    if (
      ...
      setTreatmentGoalForm(
        patientTreatmentGoal ? convertFormat(patientTreatmentGoal) : {}
      );
      setisLoading(false);
      setErrMsg('');
      // }
      ...
      <>
        ...
        >
          <Tab
            ...
            disabled={isEdit}
            tabClassName={tabKey == 'home' ? 'tab-active' : 'tab-inactive'}
          >
          ...
          <Tab
            ...
            disabled={isEdit}
            tabClassName={tabKey == 'profile' ? 'tab-active' : 'tab-inactive'}
          >
            <TreatmentGoal
              isEdit={isEdit}
              clickHandler={clickHandler}
