...
import {
  ...
  toggleSideNavigator,
} from '../../store/actions/sidenNavigatorAction';
...
const SideNavigator = ({ sideSectionShowHandler }) => {
  ...
  const userRole = useSelector(
    (state) => state.userInfoReducer?.userInfo?.data?.role
  );
  const planDetailsMapping = useSelector(
    (state) => state.sidenNavigatorReducer?.planDetailsMapping
  );
  const isSideNavigatorVisible = useSelector(
    (state) => state.sidenNavigatorReducer?.isSideNavigatorVisible
  );
  ...
  const navItemsInitialRebootSpecific = [
  ...
  useEffect(() => {
    ...
    if (isProgressPage) {
      setNavItems(navItemsInitialRebootIndependent);
    } else {
      setNavItems(navItemsInitialRebootSpecific);
    }
  }, [pathname]);

  useEffect(() => {
    if (!isProgressPage && rebootID > -1 && patientID) {
      getPlanDetailsMapping(dispatch, patientID, rebootID || 0);
    }
  }, [patientID, rebootID, pathname]);
  ...
  useEffect(() => {
    if (
      ...
      const { treatmentPlanDraft, treatmentPlanHistory, treatmentPlanLatest } =
        planDetailsMapping?.data;

      let childPlans = [];
      if (
        treatmentPlanLatest &&
        typeof treatmentPlanLatest === 'object' &&
        Object.keys(treatmentPlanLatest).length > 0
      ) {
      ...
      if (
        ...
        Array.isArray(treatmentPlanHistory) &&
        typeof treatmentPlanHistory === 'object' &&
      ...
      if (
        (CommonUtils.isAdmin(userRole[0]) || CommonUtils.isLab(userRole[0])) &&
        treatmentPlanDraft &&
        typeof treatmentPlanDraft === 'object' &&
        Object.keys(treatmentPlanDraft).length > 0
      ) {
  ...
  return (
    <div className="displayFlex">
      <aside
        className={`side-navigator-layout ${
          isSideNavigatorVisible ? 'visible' : ''
        }`}
      >
                        ...
                        item.plans.map((plan, planIndex) => (
                          <li key={planIndex} className="subplan">
                            ...
                            <Link
                              to={`${pathNamePrefix}${
                                item.path
                              }/${removeWhitespace(plan.name)}?${plan.value}`}
                              className={
                                'subplan-link ' +
                                (pathname.includes(
